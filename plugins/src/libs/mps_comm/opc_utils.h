...
public:
	...
	static bool logNodeInfo(OpcUa::Node                     node,
	                        std::shared_ptr<spdlog::logger> logger,
	                        bool                            recursive = false,
	                        int                             depth     = -1);
	// Log node info with indent
	static bool
	logNodeInfo(OpcUa::Node node, std::shared_ptr<spdlog::logger> logger, int indent, int depth);
	// Log node value ny OPC UA Node
	static bool logNodeValue(OpcUa::Node                     node,
	                         std::string                     nodeName,
	                         std::shared_ptr<spdlog::logger> logger,
	                         int                             indent);
	...
	static std::string logNodeValueArray(OpcUa::Node                     node,
	                                     std::string                     nodeName,
	                                     std::shared_ptr<spdlog::logger> logger,
	                                     int                             indent);
	// Log node array value
